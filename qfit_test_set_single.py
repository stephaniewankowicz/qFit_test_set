...
def compare_rvalues(comparison_rvalues, original_rvalues, comparison, base_dir):
    ...
    print(r_values.head())
    r_values['Rfree_Differences'] = r_values['add_bulk_Rfree_pre'] - r_values[comparison_Rfree]

    #labeling structures that will be removed due to high rfree values
    r_values['Rfree_YN'] = np.where(((r_values['add_bulk_Rfree_pre'] - r_values[comparison_Rfree]) < -0.025), 1, 0)
    #plotting the difference in rvalues
    fig=plt.figure()
    sns.lmplot('add_bulk_Rfree_pre', comparison_Rfree, data=r_values, fit_reg=False)
    slope = 1
    y_intercept = 0
...
def compare_bfactor(original_bfactor, comparison_bfactor, comparison, base_dir): 
    ...
    b_factor_all['altloc_diff'] = b_factor_all['original_num_altoc'] - b_factor_all[comparison_altloc]


    #compare altlocs
    fig = plt.figure()
    ax = sns.histplot(b_factor_all['altloc_diff'], bins=[-4, -3, -2, -1, 0, 1, 2, 3, 4], shrink=.8, color='#49316e')
    for p in ax.patches:
        height = p.get_height() # get the height of each bar
        # adding text to each bar
        ax.text(x = p.get_x()+(p.get_width()/2), y = height+0.3, s = '{:.0f}'.format(height), ha = 'center') 
    plt.xlabel('Difference in Altlocs', fontsize=12)
    ...
    #compare bfactor
    fig = plt.figure()
    sns.histplot(b_factor_all['bfactor_diff'])
...
def main():
    ...
    #read in comparison
    comparison_bfactor, comparison_rvalues, comparison_rotamer = read_files(args.base_dir, args.comparison, 60, 64) #read in cctbx: 65-69 b-factor:60, 64
